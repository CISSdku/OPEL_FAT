...
parse_long:
	...
	*nr_slots = slots;
	...
	{
		printk( KERN_ALERT "[cheon] fat_parse_long / DELETED_FLAG \n ");
		return PARSE_INVALID;
	}
	if ((*de)->attr == ATTR_EXT)
		goto parse_long;
...
parse_record:
		...
		if (de->name[0] == DELETED_FLAG)
		{
//			printk( KERN_ALERT "[cheon] fat_search_long / DELETED_FLAG \n");
			continue;
		}
...
//			printk( KERN_ALERT "[cheon] __fat_remove_entries, DELETED_FLAG \n" );
...
//		printk( KERN_ALERT "[cheon] fat_remove_entries, DELETED_FLAG \n" );
		de->name[0] = DELETED_FLAG;
		de--;
		nr_slots--;
	}
