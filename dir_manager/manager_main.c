...
void init( int dir_cnt, char **dirs, char **config_line, char *sysfs_line )
{
...
#if 1
	g_dir[ NORMAL ].dir_size        = g_dir[ NORMAL ].dir_size * 0.40;
	g_dir[ NORMAL_EVENT ].dir_size  = g_dir[ NORMAL_EVENT ].dir_size  * 0.40;
	g_dir[ PARKING ].dir_size       = g_dir[ PARKING ].dir_size * 0.40;
	g_dir[ PARKING_EVENT ].dir_size = g_dir[ PARKING_EVENT ].dir_size * 0.40;
	g_dir[ HANDWORK ].dir_size      = g_dir[ HANDWORK ].dir_size * 0.40;
	//  g_dir[ ETC ].dir_size           = control_size * ( (double)config_ratio[5] / 100 );                  //etc는 그냥 dummy
	...
	for( i = 0 ; i < dir_cnt ; i++ )
	{
		g_dir[ i ].check_portion = g_dir[ i ].dir_size * 0.25;// - ( g_dir[ i ].dir_size  / 3 );
